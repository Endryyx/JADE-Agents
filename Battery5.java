                ...
                if(msg!=null){
                   // CFP Message received. Process it
                    ...
                    threshold_=results[2];                    
                     ...
                     BatteryFunction cfp_batteries=new BatteryFunction();
                     
                     float p_nominal_=Float.parseFloat(p_nominal);
                     soc_=Float.valueOf(soc);
                     batt_name=myAgent.getAID().getName();
                                                                //(p_diff, status_, threshold_,soc_,p_nominal_,agentBattery_name)
                     batt_info_=cfp_batteries.Battery_Information(p_diff, status_, threshold_, soc_, p_nominal_,batt_name);
                    ...
                    switch(status){
                            ...
                            if(soc_<=soc_min){
                               ...
                               soc_output=soc_;
                                ...
                                if(p_diff>=p_nominal_){
                                    ...
                                    if(soc_output<soc_min){
                                        soc_output=soc_min;
                                        p_batt_output_=process.PowerDischargingTillSOCmin( ti, capacity_, soc_min, soc_);
                                    }
                            ...
                            if(soc_>=soc_max){
                               ...
                               soc_output=soc_;
                                ...
                                if(p_diff>=p_nominal_){
                                    ...
                                    if(soc_output>soc_max){
                                        soc_output=soc_max;
                                        p_batt_output_=process.PowerChargingTillSOCmax(ti, capacity_, soc_max, soc_);
                                    }
                        ...
                        case 3://Battery automatic charge/discharge
                            ...
                            if(charge==false && discharge==false){
                                ...
                                if (p_diff!=0){
                                    if(p_diff>0){
                                        System.out.println("La batería "+myAgent.getName()+" entra en fase de descarga eficiente"); 
                                        p_discharge=p_diff;
                                        soc_output=process.DischargingStatus(p_discharge,ti,capacity_, soc_min, soc_);
                                        aCD=0;
                                        p_batt_output_=p_discharge;                                        
